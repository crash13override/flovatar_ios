...
class NFTAPIClient {
    ...
    public func getScoreList(completion: @escaping (Result<[ScoreModel], Error>) -> Void) {
        
        let fullURL = URLComponents(url: url, resolvingAgainstBaseURL: false)!
        
        let _ = print(#fileID, #line, #function, fullURL.url!)
        
        loadJson(url: fullURL.url!) { result in
            switch result {
            case let .success(data):
                do {
                    let response = try JSONDecoder().decode([ScoreModel].self, from: data)
                    completion(Result.success(response))
                } catch {
                    let _ = print("Error: ", error)
                }
    ...
    public func postScore(model: ScoreModel) {

        let fullURL = URLComponents(url: url, resolvingAgainstBaseURL: false)!
        
        let _ = print(#fileID, #line, #function, fullURL.url!, "\n✅ POST Model:", model)
        
        guard let httpBody = try? JSONEncoder().encode(model) else { return }
...
//            print(#fileID, #line, "postScore", data, responce, error)
//        }
